...
**Fix:**
1. Confirm you are running the current version of `pipeline_applet.py`. For a project pipeline (the "Use manual file selection instead" box left UNCHECKED), clicking **Generate Params File...** now names and places the file automatically, inside that diff's own output folder, following the same numbered-filename pattern as every other stage output (Section 13.1 of the schema). No save dialog opens.
2. The save dialog still opens if you check "Use manual file selection instead", or if you open the Stage 5 dialog outside of a project (no Baseline/Diff header shown). This is expected - manual mode has no project folder to place the file in automatically.
...
**Cause:** Not a bug. Stage 5 (Diff) checks each side (reference and comparison) on its own. For a side where Stage 4 (Segment) has completed, Stage 5 uses that side's classified output. For a side where Stage 4 has not run yet, Stage 5 falls back to that side's Stage 3 (Cleanup) output instead of blocking the diff. The two sides do not need to be in the same state - a diff can run with Segment done on one side only.

**Fix:** No fix is needed if this is expected. The compared points are the same cloud either way - Stage 4 (Segment) only adds a `classification` field on top of Stage 3's own output, so which stage a side's cloud came from does not change the M3C2 result.

1. To confirm which stage supplied each side's cloud for a given diff run, check `project.json` for that diff pipeline. The diff stage entry records `reference_source_stage` and `comparison_source_stage`, each set to `"segment"` or `"cleanup"`.
2. To force both sides to use Stage 4 (Segment)'s classified output, run Stage 4 (Segment) for the side that is still falling back to `"cleanup"`, then re-run Stage 5 (Diff).
